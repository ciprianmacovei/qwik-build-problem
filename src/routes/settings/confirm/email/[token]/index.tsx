...
export const useEmailConfirm = routeLoader$(async (requestEvent) => {
  // This code runs only on the server, after every navigation
  try {
    const res = await fetch(
      `http://backend:8000/api/user/change/email/${requestEvent.params.token}`,
      {
        method: "GET",
        headers: {
...
export default component$(() => {
  ...
  const emailConfirmSignal = useEmailConfirm();
  ...
  return (
    <>
      {emailConfirmSignal.value.status === 200 && (
